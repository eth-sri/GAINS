...
class HybridZonotope(AbstractElement):
    ...
    def __truediv__(self, other: Union[Tensor, int, float]) -> "HybridZonotope":
        if isinstance(other, torch.Tensor) or isinstance(other, float) or isinstance(other, int) or isinstance(other, torch.Tensor):
            return HybridZonotope(self.head / other,
                                  None if self.beta is None else self.beta / other.abs(),
    ...
    def global_avg_pool2d(self,output_size) -> "HybridZonotope":
        new_head = F.adaptive_avg_pool2d(self.head, output_size)
        new_beta = None if self.beta is None else F.adaptive_avg_pool2d(self.beta.view(-1, *self.head.shape[1:]), output_size)
        new_errors = None if self.errors is None else F.adaptive_avg_pool2d(self.errors.view(-1, *self.head.shape[1:]), output_size).view(-1, *new_head.shape)
        return HybridZonotope(new_head, new_beta, new_errors, self.domain)
    ...
    def cat_tensor(self,tensor, dim=0):
        dtype = self.head.dtype
        device = self.head.device
        new_head = torch.cat([torch.ones_like(self.head[:, :1, :, :],dtype = dtype) * tensor,self.head],dim=1).to(device)

        if self.beta is None:
            new_beta = None 
        else:
            new_beta = torch.cat([torch.zeros_like(self.beta[:, :1, :, :],dtype = dtype),self.beta],dim=1).to(device)

        if self.errors == None:
            new_dict = None
        else:#should not happen at the moment
            print("zonotope.py in cat_tensor")
            exit()

        return HybridZonotope(new_head, new_beta, new_dict, self.domain)
    ...
    def sigmoid(self, deepz_lambda:Union[None,Tensor]=None, bounds:Union[None,Tensor]=None) -> Tuple["HybridZonotope",Union[Tensor,None]]:
        lb, ub = self.concretize()
        D = 1e-6

        if self.domain == "box":
            min_relu, max_relu = torch.sigmoid(lb), torch.sigmoid(ub)
    ...
    def log(self, deepz_lambda:Union[None,Tensor]=None, bounds:Union[None,Tensor]=None,mode = None) -> Tuple["HybridZonotope",Union[Tensor,None]]:
        lb, ub = self.concretize()
        D = 1e-6
        ...
        if bounds is not None:
            ...
            ub = torch.min(ub_refined, ub)
        if not (lb >= 0).all():
            lb = torch.max(lb, torch.zeros_like(lb).to(lb.device))
        assert (lb >= 0).all()
        ...
        log_lb_0 = torch.where(log_lambda < log_lambda_s, (ub+D).log() - ub * log_lambda, (lb+D).log() - lb * log_lambda)
    ...
    def exp(self, deepz_lambda:Union[None,Tensor]=None, bounds:Union[None,Tensor]=None) -> Tuple["HybridZonotope",Union[Tensor,None]]:
        ...
        for _ in range(2): #First go with minimum area (while non negative lb)/ provided slopes. If this leads to negative/zero lower bounds, fall back to minimum slope
            ...
            exp_mu = 0.5 * (exp_lambda * (1 - (exp_lambda+10*D).log()) + exp_ub_0)
            exp_delta = 0.5 * (exp_ub_0 - exp_lambda * (1 - (exp_lambda+10*D).log()))
    ...
    def abs(self, deepz_lambda:Union[None,Tensor]=None, bounds:Union[None,Tensor]=None,mode = False) -> Tuple["HybridZonotope",Union[Tensor,None]]:
        lb, ub = self.concretize()
        ...
        if self.domain == "box":
            ...
            min_abs, max_abs = lb.abs(), ub.abs()
            min_abs[is_under],max_abs[is_under] = max_abs[is_under],min_abs[is_under]
            min_abs[is_cross],max_abs[is_cross] = 0.0, torch.maximum(min_abs[is_cross],max_abs[is_cross])
            min_abs,max_abs = min_abs.view(*shp),max_abs.view(*shp)
            if mode:
                return min_abs, max_abs
            return HybridZonotope.construct_from_bounds(min_x= min_abs,max_x =max_abs,dtype = self.dtype,domain = self.domain)
            ...
            if mode:
                out = out.flatten()
                delta = out.get_errors().sum(1).abs().sum()
                alpha = out.head.sum()
                return alpha - delta, alpha + delta
            else:
                out
        else:
            raise RuntimeError("Error applying Abs with unkown/unimplemented domain: {}".format(self.domain))
    ...
    def add(self, summand_zono:Union["HybridZonotope",int], shared_errors:int=0) -> "HybridZonotope":

        if isinstance(summand_zono, int):
            dtype = self.head.dtype
            device = self.head.device
            new_head = self.head + summand_zono
            new_beta = self.beta
    ...
    def get_gaussian_likelihood_dist(self, data:Tensor) -> Tensor:
        #returns worst case likelihood in concrete domain
        low, upp = self.concretize()
        low_temp,upp_temp = (low - data).abs(),(upp - data).abs()
        out = torch.where(upp_temp >= low_temp,upp, low)
        return out

    def get_gaussian_kl_std(self):
        #returns worst case likelihood in concrete domain
        D = 1e-8
        low, upp = self.concretize()
        low = F.relu(low) + D
        ...
        low_temp = low.pow(2) - 2 * low.log()
        upp_temp = upp.pow(2) - 2 * upp.log()
        out = torch.where(upp_temp >= low_temp,upp, low)
        return out
