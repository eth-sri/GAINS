...
class BaseScheduler(object):
    def __init__(self, max_eps, opt_str,init_value =0.0):
...
class SmoothedScheduler(BaseScheduler):
    ...
    def __init__(self, max_eps, opt_str,init_value = 0.0):
        super(SmoothedScheduler, self).__init__(max_eps, opt_str,init_value)
    ...
    def step_batch(self, verbose=False):
        ...
        if self.is_training and self.schedule_length > 0:
            self.batch += 1
        ...
        if self.is_training:
            ...
            tu = np.log((final_step - mid_step))/np.log(beta -1)
            alphau = (final_value - init_value) / ((final_step - mid_step) * beta * tu + (final_step- mid_step) * tu)
            exp_value = init_value + alpha * float(step - init_step) ** beta
            linear_value = min(mid_value + alphau * np.log(abs(float(step - mid_step)))/np.log(beta), final_value)
            self.eps = is_ramp * ((1.0 - is_linear) * exp_value + is_linear * linear_value) + (1.0 - is_ramp) * init_value
        '''
...
if __name__ == "__main__":
    s = SmoothedScheduler(1.5/255, "start=50,length=420,mid=0.9,beta=8.0")
    epochs = 500
    batches = 1
    loss = 1.0
    eps = []
