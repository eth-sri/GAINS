...
def odeint(func, y0, t, *, rtol=1e-7, atol=1e-9,step_size = None,method=None, options=None, event_fn=None,steady_state = None, adaptive_step_factor = None,running_mean_step = None,traversed_states = None):
    ...
    Args:
        ...
        event_fn: Function that maps the state `y` to a Tensor. The solve terminates when
            ...
            `t` are ignored.
        steady_state: whether or not to compute the value of the ss integral from T up until T+1
        adaptive_step_method: choose between normal update rule for adaptive solver or multiplicative / limited at Tend
    ...
    solver = SOLVERS[method](func=func, y0=y0, rtol=rtol, atol=atol,step_size= step_size, adaptive_step_factor = adaptive_step_factor,running_mean_step = running_mean_step, **options)

    if steady_state == 0:
        solution,n_tot, n_acc, avg_step,steady_state = solver.integrate_ss(t)
        return solution,n_tot,n_acc, avg_step,steady_state

    if steady_state == 1:
        solution,n_tot, n_acc, avg_step,steady_state = solver.integrate_path(t)
        return solution,n_tot,n_acc, avg_step,steady_state

    if event_fn is None and traversed_states == None:
        solution,liste = solver.integrate(t)
    elif traversed_states != None:
        solution = solver.integrate_traj(traversed_states)
        return solution
