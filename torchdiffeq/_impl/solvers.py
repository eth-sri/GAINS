...
class AdaptiveStepsizeODESolver(metaclass=abc.ABCMeta):
    def __init__(self, dtype, y0, norm, **unused_kwargs):
        self.step_size = unused_kwargs.pop('step_size')
    ...
    @abc.abstractmethod
    def _advance_mod(self, two_tend):
        raise NotImplementedError
    @abc.abstractmethod
    def _advance_traj(self, states):
        raise NotImplementedError
    ...
    def integrate(self, t):
        ...
        for i in range(1, len(t)):
            if self.adaptive_step_factor != None:
                solution[i] = self._advance_mod(t[i])
            else:
                solution[i] = self._advance(t[i])
                steps += self.n_tot_steps
        return solution, self.list

    def integrate_traj(self, states):
        solution = self._advance_traj(states)
        return solution
...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    order: int
    #CHANGED STEP_SIZE DEFAULT FROM NONE TO 0.1
    def __init__(self, func, y0, step_size, grid_constructor=None, interp="linear", perturb=False, **unused_kwargs):
        self.atol = unused_kwargs.pop('atol')
        unused_kwargs.pop('rtol', None)
        unused_kwargs.pop('norm', None)
        unused_kwargs.pop('adaptive_step_factor',None)
        unused_kwargs.pop('running_mean_step',None)
        _handle_unused_kwargs(self, unused_kwargs)
        del unused_kwargs
    ...
    def integrate(self, t):
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            ...
            while j < len(t) and t1 >= t[j]:
                ...
                j += 1
            y0 = y1
        liste = [(len(time_grid)-1),(len(time_grid)-1),(time_grid[1:]- time_grid[0:-1]).mean().cpu().detach().numpy()]
