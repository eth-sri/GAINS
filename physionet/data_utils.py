...
def split_data_extrap(data_dict, dataset = "",ratio = 1.0,data_mode = 0):
	device = get_device(data_dict["data"])
	#pdb.set_trace()

	#get for each entry the final time
	#what do I do when final int time in batch < final observed value

	split_dict = {}
	#get last tp and data with mask for each sample in batch
	predict_mask = data_dict["mask"].sum(2)

	target_tp = []

	if data_mode != 4:

		target_data = torch.zeros_like(data_dict["data"])
		target_mask = torch.zeros_like(data_dict["mask"])
		for b in range(predict_mask.shape[0]):
			idx = torch.where(predict_mask[b] > 0)[0][-1]
			target_tp.append(data_dict["time_steps"][idx].item())
			
			#set targets
			target_data[b,idx] = data_dict["data"][b,idx]
			target_mask[b,idx] = data_dict["mask"][b,idx]

			#remove from training 
			data_dict["data"][b,idx] = 0.
			data_dict["mask"][b,idx] = 0.

		liste = []
		for x in set(target_tp):
			liste.append(x)
		liste.sort()
		target_tp = torch.tensor(liste).to(data_dict["data"].device).type(data_dict["data"].dtype)
		idx = torch.where(target_mask.sum((0,2)) > 0)[0]
		target_data = target_data[:,idx]
		target_mask = target_mask[:,idx]
		#set targets
		split_dict["tp_to_predict"] = target_tp
		split_dict["data_to_predict"] = target_data
		split_dict["mask_predicted_data"] = target_mask


		n_observed_tp = torch.where(data_dict["time_steps"]<= ratio)[0].shape[0] 
		time_steps = data_dict["time_steps"][:n_observed_tp].clone()
		obs_data = data_dict["data"][:,:n_observed_tp,:].clone()
		obs_mask = data_dict["mask"][:, :n_observed_tp].clone()
		#ADD ADDITIONAL INTEGRATION POINT
		if 1. not in time_steps:
			time_steps = torch.cat((time_steps,torch.ones_like(time_steps[0:1])))
			obs_data = torch.cat((obs_data,torch.zeros_like(obs_data[:,0:1])),1)
			obs_mask = torch.cat((obs_mask,torch.zeros_like(obs_mask[:,0:1])),1)

		split_dict["observed_data"] = obs_data
		split_dict["observed_mask"] = obs_mask
		split_dict["observed_tp"] = time_steps
		split_dict["observed_init"] = data_dict["init"].clone()
		...
		target_mask = torch.zeros_like(data_dict["mask"])
		for b in range(predict_mask.shape[0]):
			idx = torch.where(predict_mask[b] > 0)

			last_time = data_dict["time_steps"][idx[0][-1]]
			import pdb
			time_idx_obs = torch.where((data_dict["time_steps"]<= (last_time-0.25)) * (predict_mask[b] > 0) ) 
			time_idx_pred = torch.where((data_dict["time_steps"]> (last_time-0.25)) * (predict_mask[b] > 0) ) 

			for iii in time_idx_pred[0]:
				target_tp.append(data_dict["time_steps"][iii].item())
			
				#set targets
				target_data[b,iii] = data_dict["data"][b,iii]
				target_mask[b,iii] = data_dict["mask"][b,iii]

				#remove from training 
				data_dict["data"][b,iii] = 0.
				data_dict["mask"][b,iii] = 0.
		...
		obs_data = data_dict["data"][:,:n_observed_tp,:].clone()
		obs_mask = data_dict["mask"][:, :n_observed_tp].clone()
		idx = torch.where(obs_mask.sum((0,2)) > 0)[0]
	...
	#quick hack to test MAE on final datapoinnts with full observation training
	#if ratio == 1.0:
	#	n_observed_tp = 0

	#remain_data = data_dict["data"][:,n_observed_tp:,:].clone()
	#predict_mask = data_dict["mask"][:, n_observed_tp:].clone()
	#tp_to_predict = data_dict["time_steps"][n_observed_tp:].clone()

	#mask_per_tp = predict_mask.sum(2)
	#sparse_mask = torch.zeros_like(predict_mask)

	#for b in range(predict_mask.shape[0]):
	#	idx = torch.where(mask_per_tp[b] > 0)[0]
	#	if len(idx)<3:
	#		for iii in idx:
	#			sparse_mask[b,iii] = predict_mask[b,iii]
	#	else :
	#		idx = idx[-3:]
	#		for iii in idx:
	#			sparse_mask[b,iii] = predict_mask[b,iii]
	#		#sparse_mask[b,idx[0]] = predict_mask[b,idx[0]]
	#		#sparse_mask[b,idx[-1]] = predict_mask[b,idx[-1]]
	#		#maxi = mask_per_tp[b].argmax()
	#		#sparse_mask[b,maxi] = predict_mask[b,maxi]

	#idx = torch.where(sparse_mask.sum((0,2))>0)
	#tp_to_predict = tp_to_predict[idx]
	#split_dict["tp_to_predict"] = tp_to_predict[idx]

	#sparse_mask = sparse_mask.transpose(1,0)[idx].transpose(1,0)
	#remain_data = remain_data.transpose(1,0)[idx].transpose(1,0)
	#split_dict["data_to_predict"] = remain_data.transpose(1,0)[idx].transpose(1,0)
	#split_dict["mask_predicted_data"] = sparse_mask.transpose(1,0)[idx].transpose(1,0)
...
def split_data_interp(data_dict):
	...
	split_dict = {"observed_data": data_dict["data"].clone(),
				"observed_init": data_dict["init"].clone(),
...
def split_and_subsample_batch(data_dict, args, data_type = "train"):
	if data_type == "train":
		# Training set
		if args.extrap:
			processed_dict = split_data_extrap(data_dict, dataset = args.dataset,ratio=args.ratio,data_mode = args.data_mode)
		else:
