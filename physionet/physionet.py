...
def get_data_mean_std(records):
	device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

	observed_values =[[] for i in physio_params]

	for b, (record_id, tt, vals, mask, labels) in enumerate(records):
		n_features = vals.size(-1)
		for i in range(n_features):
			if i in [1,3,14,15,24,34]:
				continue
			non_missing_vals = vals[:,i][mask[:,i] == 1]
			if i == 36:
				non_missing_vals = torch.clamp(non_missing_vals,32,45)
			elif i == 39:
				non_missing_vals = torch.clamp(non_missing_vals,0,2000)
			elif i == 40:
				non_missing_vals = torch.clamp(non_missing_vals,0,60)
			if i == 31:
				non_missing_vals = torch.clamp(non_missing_vals,0,14)
			for x in non_missing_vals:
				observed_values[i].append(x.item())

	data_mean = []
	data_std = []
	for i in range(int(len(physio_params))):
		liste = observed_values[i]
		if i in [1,3,14,15,24]:
			mean = 0.
			std = 1.
		elif i in [34]:
			mean = 0
		...
		data_std.append(std)
	data_mean = torch.tensor(data_mean).to(device)
	data_std = torch.tensor(data_std).to(device)
	return data_mean, data_std

def get_data_features(records):
	device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
			...
			for x in non_missing_vals:
				if i == 39:
					if x > 1200:
						...
						c2 += 1
					observed_values[i].append(torch.clamp(x,0,1200).item())
				elif i == 40:
					if x > 60:
						c3 += 1
					elif x <0:
						c4 += 1
					observed_values[i].append(torch.clamp(x,0,60).item())
	print(c1,c2,c3,c4)
	for i in range(int(len(physio_params))):
		if i not in [39,40]:
			continue
		name = physio_params[i]
		print(name,np.min(observed_values[i]),np.max(observed_values[i]))
		plt.hist(observed_values[i],20)
		plt.title("Distribution of Feature: {0}, {1}".format(name,i))
		plt.savefig("/local/home/mzeqiri/node-cert/node models/physionet/PhysioNet/images/features_{0}_{1}.png".format(name,i))
		plt.clf()
	pdb.set_trace()
...
def variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	combined_init = torch.zeros([len(batch),  6]).to(device)
	...
	for b, (record_id, tt, vals, mask, labels) in enumerate(batch):
		...
		#clip some features
		vals[:,15] = torch.clamp(vals[:,15],3.0,15.0)
		vals[:,15][torch.where(mask[:,15]== 0)] = 0.
		vals[:,31] = torch.clamp(vals[:,31],0.0,14.0)
		vals[:,36] = torch.clamp(vals[:,36],32.0,45.0)
		vals[:,36][torch.where(mask[:,36]== 0)] = 0.
		vals[:,39] = torch.clamp(vals[:,39],0.0,2000.0)
		vals[:,40] = torch.clamp(vals[:,40],0.0,60.0)

		#get additional init obs info
		idx = torch.where(mask.sum(1) > 0)[0][-1]
		temp_mask = mask.clone()
		temp_mask[idx] = 0.
		combined_init[b][4] = vals[:,15][torch.where((temp_mask[:,15] > 0)*(tt <= args.ratio * 48.0))].mean()
		combined_init[b][5] = (vals[:,24][torch.where((temp_mask[:,24] > 0)*(tt <= args.ratio * 48.0))].sum() > 0).float()
		#combined_init[b][6] = vals[:,31][torch.where((temp_mask[:,31] > 0) *(tt <= args.ratio * 48.0))].mean()

		if torch.isnan(combined_init[b][4]):
			combined_init[b][4] = 0 
		#if torch.isnan(combined_init[b][6]):
		#	combined_init[b][6] = 0  #should either way not happen

		combined_vals[b, indices] = vals[:,4:]
		combined_mask[b, indices] = mask[:,4:]
		combined_init[b][0:4] = vals[0,:4]

		if labels is not None:
			combined_labels[b] = labels

	combined_vals, _, _ = utils.normalize_masked_data(combined_vals, combined_mask, 
		att_min = data_min[4:], att_max = data_max[4:])

	#combined_vals = torch.cat([combined_vals[:,:,0:11],combined_vals[:,:,12:20],combined_vals[:,:,21:27],combined_vals[:,:,28:]],dim = 2)
	#combined_mask = torch.cat([combined_mask[:,:,0:11],combined_mask[:,:,12:20],combined_mask[:,:,21:27],combined_mask[:,:,28:]],dim = 2)

	combined_vals = torch.cat([combined_vals[:,:,0:11],combined_vals[:,:,12:20],combined_vals[:,:,21:]],dim = 2)
	combined_mask = torch.cat([combined_mask[:,:,0:11],combined_mask[:,:,12:20],combined_mask[:,:,21:]],dim = 2)

	combined_init[:,0:4] = (combined_init[:,0:4] -data_min[0:4])/ data_max[0:4]

	# I WANT ALL TIMES TO BE IN [0,1]
	combined_tt = combined_tt / 48.00
